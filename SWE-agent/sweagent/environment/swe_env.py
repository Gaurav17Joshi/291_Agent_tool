...
#     def start(self) -> None:
#         """Start the environment and reset it to a clean state."""
#         from sweagent.detailed_timing_tracker import get_detailed_timing_tracker

#         tracker = get_detailed_timing_tracker()

#         # Phase 1: Container/deployment provisioning
#         key_provision = tracker.start_entry("environment_provisioning", detail="deployment init (docker/container start)")
#         self._init_deployment()
#         tracker.end_entry(key_provision, "environment_provisioning")

#         # Phase 2: Workspace initialization (repo clone, reset, post-startup commands)
#         key_workspace = tracker.start_entry("workspace_initialization", detail="repo clone, reset, post-startup commands")
#         self.reset()
#         for command in self._post_startup_commands:
#             self.communicate(command, check="raise", timeout=self.post_startup_command_timeout)
#         tracker.end_entry(key_workspace, "workspace_initialization")
