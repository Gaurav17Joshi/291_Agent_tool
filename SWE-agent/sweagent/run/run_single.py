...
# from sweagent.timing_tracker import get_timing_tracker, reset_timing_tracker
...
#     # def run(self):
#     # # Initialize timing tracker for single run
#     #     reset_timing_tracker()
#     #     tracker = get_timing_tracker()
#     #     tracker.start_task(self.problem_statement.id)
...
#     #     # End tracking and save timing data
#     #     tracker.end_task()
#     #     timing_output_path = self.output_dir / "timing_profile.json"
#     #     tracker.save_to_file(timing_output_path)
#     #     self.logger.info("Timing profile saved to %s", timing_output_path)
...
#         env_start_timer = tracker.start_timer("environment_setup", "env.start()")
#         self.env.start()
#         tracker.end_timer(env_start_timer, "environment_setup")
...
#         # TIMING: environment teardown
#         env_close_timer = tracker.start_timer("environment_teardown", "env.close()")
#         self.env.close()
#         tracker.end_timer(env_close_timer, "environment_teardown")
...
from sweagent.utils.log import add_file_handler, get_logger
from sweagent.timing_tracker import get_timing_tracker, reset_timing_tracker
from sweagent.detailed_timing_tracker import get_detailed_timing_tracker, reset_detailed_timing_tracker
...
class RunSingle:
    ...
    def run(self):
        ...
        reset_timing_tracker()
        reset_detailed_timing_tracker()

        tracker = get_timing_tracker()
        detailed = get_detailed_timing_tracker()

        tracker.start_task(self.problem_statement.id)
        detailed.start_task(self.problem_statement.id)

        # TIMING: process_bootstrap — everything before env.start()
        bootstrap_key = detailed.start_entry("process_bootstrap", detail="pre-environment setup")

        self._chooks.on_start()
        self.logger.info("Starting environment")

        detailed.end_entry(bootstrap_key, "process_bootstrap")

        # TIMING: environment setup (original tracker)
        env_start_timer = tracker.start_timer("environment_setup", "env.start()")
        # Note: env.start() now internally records environment_provisioning and workspace_initialization
        ...
        # TIMING: cleanup_and_teardown (both trackers)
        env_close_timer = tracker.start_timer("environment_teardown", "env.close()")
        teardown_key = detailed.start_entry("cleanup_and_teardown", detail="env.close()")
        self.env.close()
        tracker.end_timer(env_close_timer, "environment_teardown")
        detailed.end_entry(teardown_key, "cleanup_and_teardown")
        ...
        detailed_output_path = self.output_dir / "detailed_timing_profile.json"
        detailed.save_to_file(detailed_output_path)
        self.logger.info("Detailed timing profile saved to %s", detailed_output_path)
...
def run_from_cli(args: list[str] | None = None):
    ...
    run_from_config(BasicCLI(RunSingleConfig, help_text=help_text).get_config(args))
