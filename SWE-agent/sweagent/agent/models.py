...
class GenericAPIModelConfig(PydanticBaseModel):
    ...
    """If True, inject cache_control markers into the system message (and tool definitions) to
    enable prompt caching. This can significantly reduce input token costs when the system prompt
    is large and repeated across many turns.
    Supported by Anthropic (claude-*), Bedrock, and Deepseek models — these require explicit
    cache_control annotations. OpenAI models cache automatically without needing this flag.
...
class LiteLLMModel(AbstractModel):
    ...
    @staticmethod
    def _count_cache_controls(messages: list[dict], tools: list[dict] | None) -> int:
        """Count existing cache_control blocks across messages and tool definitions."""
        count = 0
        for msg in messages:
    ...
    def _apply_prompt_caching(self, messages: list[dict], tools: list[dict] | None = None) -> tuple[list[dict], list[dict] | None]:
        """Inject cache_control markers for Anthropic-style prompt caching.

        Anthropic allows a maximum of 4 cache_control blocks per request. This method
        respects that limit by counting existing blocks (e.g. from CacheControlHistoryProcessor)
        before adding new ones. New blocks are added in priority order:

        1. System message — large and constant across all turns.
        2. Last tool definition — large and constant across all turns.
        3. Rolling conversation history — marks the most recent complete message before
           the current new observation. Since history only grows by appending, Anthropic's
           exact-prefix matching makes everything up to this checkpoint a cache HIT on
           the next turn.

        If CacheControlHistoryProcessor is also active, it already handles (3) and uses
        2 of the 4 slots, leaving 2 for (1) and (2) — which fits exactly.

        Returns (modified_messages, modified_tools).
        """
        _ANTHROPIC_MAX_CACHE_BLOCKS = 4
        modified = list(messages)
        modified_tools = tools

        existing = self._count_cache_controls(modified, modified_tools)
        budget = max(0, _ANTHROPIC_MAX_CACHE_BLOCKS - existing)
        ...
        if budget > 0 and tools:
            modified_tools = list(tools)
            last_tool = dict(modified_tools[-1])
            last_tool["cache_control"] = {"type": "ephemeral"}
            modified_tools[-1] = last_tool
            budget -= 1

        # 3. Rolling conversation cache: mark the last non-system message with
        # non-empty content, excluding the final (newest) message.
        # On the next turn, everything up to this checkpoint is a cache HIT so only
        # the new observation is charged at full input-token price.
        if budget > 0:
            non_system_indices = [i for i, m in enumerate(modified) if m.get("role") != "system"]
            if len(non_system_indices) >= 2:
                for candidate_idx in reversed(non_system_indices[:-1]):
                    msg = modified[candidate_idx]
                    content = msg.get("content", "")
                    ...
                    if isinstance(content, str):
                        ...
                        last_block["cache_control"] = {"type": "ephemeral"}
                        new_content[-1] = last_block
                    new_msg = dict(modified[candidate_idx])
                    new_msg["content"] = new_content
                    modified[candidate_idx] = new_msg
                    break

        return modified, modified_tools
    ...
    def _single_query(
        ...
        for message in messages_no_cache_control:
            ...
            # Also strip cache_control from within content blocks (added by prompt caching)
        ...
        # Inject cache_control markers for prompt caching (Anthropic, Bedrock, Deepseek)
        if self.config.enable_prompt_caching:
            messages, cached_tools = self._apply_prompt_caching(
                messages, tools=extra_args.get("tools")
            )
            if cached_tools is not None:
                extra_args["tools"] = cached_tools
