...
#     #     tracker = get_timing_tracker()
#     #     tracker.start_task(instance.problem_statement.id)
...
class RunBatch:
    ...
    def _run_instance(self, instance: BatchInstance) -> AgentRunResult:
        ...
        # Start both trackers for this task
        tracker = get_timing_tracker()
        detailed = get_detailed_timing_tracker()

        tracker.start_task(instance.problem_statement.id)
        detailed.start_task(instance.problem_statement.id)

        # TIMING: process_bootstrap — agent creation, config writing, hook setup
        bootstrap_key = detailed.start_entry("process_bootstrap", detail="agent creation, config, hook setup")
        ...
        try:
            ...
            # Note: env.start() now internally records environment_provisioning
            # and workspace_initialization in the detailed tracker
            env_start_timer = tracker.start_timer("environment_setup", "env.start()")
            env.start()
            tracker.end_timer(env_start_timer, "environment_setup")
